<template>
	<div class="style-form-page">
		<PageHeader
			:title="isNew ? 'New Style' : (doc?.name || 'Style')"
			backRoute="/styles"
		>
			<template #actions>
				<button
		...
		<div v-else-if="docError && !doc && !isNew" class="error-state">
			...
			<p>{{ docError }}</p>
			<button class="btn btn-secondary" @click="$router.push('/styles')">Back</button>
		</div>
		...
		<template v-else>
			...
			<div class="detail-card" :class="{ 'detail-card--readonly': isReadonly }">
				<div class="detail-card-header">
					...
					<div>
						<h3 class="detail-card-title">Style Details</h3>
						<p class="detail-card-subtitle">Basic information about this style</p>
					</div>
				</div>
				<div class="form-grid">
					<FormField
						label="Style Code"
						:required="true"
						:error="validationErrors.style_code"
						:readonly="isReadonly"
					>
						<input
							type="text"
							...
							:disabled="isReadonly"
							@input="form.style_code = $event.target.value"
						/>
					</FormField>
					...
					<FormField
						label="Style Name"
						:required="true"
						:error="validationErrors.style_name"
						:readonly="isReadonly"
					>
						<input
							type="text"
							...
							:disabled="isReadonly"
							@input="form.style_name = $event.target.value"
						/>
					</FormField>
					...
					<FormField
						label="Machine Frame"
						:required="true"
						:error="validationErrors.machine_frame"
						:readonly="isReadonly"
					>
						<LinkField
							:modelValue="form.machine_frame || ''"
							doctype="Machine Frame"
							placeholder="Select Machine Frame"
							:disabled="isReadonly"
							@update:modelValue="(val) => form.machine_frame = val"
						/>
					</FormField>
					...
					>
						<input
							...
							placeholder="GG"
							:disabled="isReadonly"
							@input="form.gg = $event.target.value"
						/>
					</FormField>
...
const isReadonly = computed(() => {
	if (isNew.value) return !permCanCreate('Style')
	return !permCanWrite('Style')
})

const canDelete = computed(() => !isNew.value && permCanDelete('Style'))
...
const fields = [
	...
	{ fieldname: 'machine_frame', fieldtype: 'Link', label: 'Machine Frame', reqd: 1 },
...
async function handleSave() {
	...
	try {
		...
		toast.success('Saved', 'Style saved successfully')
		if (isNew.value && result?.name) {
			router.replace(`/styles/${encodeURIComponent(result.name)}`)
		}
	} catch (e) {
	...
	confirm.require({
		...
		accept: async () => {
			try {
				...
				toast.success('Deleted', 'Style deleted')
				router.push('/styles')
			} catch (e) {
...
/* Processes (full width) */
.style-form-page > :deep(.item-card-list:last-child) {
	margin-bottom: var(--space-lg);
