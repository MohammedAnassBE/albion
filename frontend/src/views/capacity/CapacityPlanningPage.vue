    ...
    <div class="capacity-planning">
                ...
                <div class="filter-rows">
                    <div class="filter-row" v-show="!isZoomedOut">
                        ...
                        <div class="filter-field">
                            ...
                            <LinkField :modelValue="selectedOrder" doctype="Order" placeholder="Select Order" :filters="orderFilters" @update:modelValue="onOrderLinkChange" />
                        </div>
                        <div class="filter-field">
                            <label class="filter-label">Process</label>
                            <select class="form-control" v-model="selectedProcess" @change="onProcessChange">
                                <option value="">Select Process</option>
                                <option v-for="proc in availableProcesses" :key="proc.name" :value="proc.name">{{ proc.process_name }}</option>
                            </select>
                        </div>
                        <div class="filter-field">
                    ...
                    <div class="filter-row">
                        <div class="action-group">
                            ...
                            <button class="btn btn-primary" @click="doSaveAllocations" v-show="!isZoomedOut">Save</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-if="showBulkShiftModal" class="modal-overlay" @click="closeBulkShiftModal">
            <div class="modal-content bulk-shift-modal" @click.stop>
                ...
                <div class="form-group">
                    <label>Date</label>
                    <input type="date" class="form-control" v-model="bulkShiftDate" @change="populateBulkShiftTable" />
                </div>
...
import {
    ...
    getAllAllocations as apiGetAllAllocations,
...
function onOrderLinkChange(val) {
...
startDate.value = _sevenDaysAgo.toISOString().split('T')[0];
const _fiveMonthsLater = new Date(_now);
_fiveMonthsLater.setMonth(_fiveMonthsLater.getMonth() + 5);
endDate.value = _fiveMonthsLater.toISOString().split('T')[0];
...
function executeMoveGroup(groupData, targetDays, newMachineId) {
    ...
    if (remaining > 0 && minutesPerUnit > 0) {
        ...
        if (remaining > 0) {
            toast.warn(`${remaining} units could not be placed (no available days)`);
        }
    }
...
function moveGroup(groupData, newMachineId, newStartDate) {
    ...
    if (groupAllocs.length === 0) {
        toast.error('Group allocations not found');
        return;
    }
    ...
    for (const { newDate } of targetDays) {
        if (!dates.includes(newDate)) {
            toast.error('Move would place days outside calendar range');
            return;
        }
        if (isPastDate(newDate)) {
            toast.error('Cannot move to past date');
            return;
        }
    }
    ...
    if (newMachineId === groupData.machine_id) {
        ...
        for (const otherGroup of machineGroups) {
            ...
            if (newStartDate > otherGroup.start_date && newStartDate <= otherGroup.end_date) {
                toast.warn('Cannot drop in the middle of an existing group. Drop at the start of the group to cascade shift.');
                return;
            }
        }
    }
    ...
    if (outOfBounds) {
        toast.error('Cannot move: some groups would be pushed beyond the calendar date range. Extend end date first.');
        return;
    }
...
function confirmEditGroup() {
    ...
    if (item) {
        ...
        if (currentAllocated + adjustment > item.quantity) {
            toast.error('Cannot exceed order quantity');
            return;
        }
    }
    ...
    toast.success('Group quantity updated');
    closeEditGroupModal();
}
...
function confirmSplitGroup() {
    ...
    if (splitIdx <= 0) {
        toast.error('Invalid split date');
        return;
    }
    ...
    toast.success('Group split successfully');
...
function confirmShiftByDays() {
    ...
    if (shiftedAllocations.length > 0 || pushMoveResults.length > 0 || pullMoveResults.length > 0) {
        ...
        toast.success(`Shifted ${groupCount} group(s) and moved group by ${shiftByDaysCount.value} day(s)`);
    } else {
        ...
        toast.success(`Group shifted by ${shiftByDaysCount.value} day(s)`);
    }
...
function deleteGroup() {
    ...
    if (affected.length > 0) {
        ...
        confirm.require({
            header: 'Delete Group',
            message: `Delete group: ${group.allocs.length} day(s)?`,
            acceptLabel: 'Delete', rejectLabel: 'Cancel',
            accept() {
                saveAction('delete_group', { allocations: undoData });
                const blockId = addDeletedBlock(group, undoData);
                actionHistory.value[actionHistory.value.length - 1].data.deletedBlockId = blockId;
                toast.success(`Deleted group: ${group.allocs.length} day(s)`);
            },
            reject() {
...
function confirmBackfill() {
    ...
    actionHistory.value[actionHistory.value.length - 1].data.deletedBlockId = blockId;
    toast.success(`Deleted group and shifted ${shiftedAllocations.length} allocation(s) backward`);
    closeBackfillModal();
}
...
function autoReflowAfterAlteration(dateStr, machineId, alterationType, minutes) {
    console.log('[autoReflow] dateStr:', dateStr, 'machineId:', machineId,
        'alterationType:', alterationType, 'minutes:', minutes);
    ...
    for (const mId of affectedMachines) {
        ...
        console.log('[autoReflow] machine:', mId, 'cellAllocs:', cellAllocs.length,
            'getShiftMinutes:', getShiftMinutes(dateStr, mId),
            'isDefaultWeekdayOff:', isDefaultWeekdayOff(dateStr),
            'getEffectiveMinutes:', getEffectiveMinutes(dateStr, mId));
    ...
    if (allAdded.length || allRemoved.length || allModified.length) {
        ...
        if (totalUnplaced > 0) {
            toast.warn('Reflow complete: ' + parts.join(', '));
        } else {
            toast.success('Reflow complete: ' + parts.join(', '));
        }

        if (totalUnplaced > 0) {
            toast.warn(`Warning: ${totalUnplaced} qty could not be placed`);
        }
    }
}
...
function onDrop(event, machineId, dateStr) {
    ...
    if (remainingQuantity < MIN_BATCH_SIZE) {
        toast.error(`Minimum batch size is ${MIN_BATCH_SIZE}`);
        return;
    }

    if (isOffDay(dateStr)) {
        toast.warn('Warning: Allocating to a weekly off day');
    }
...
function moveAllocation(alloc, newMachineId, newDateStr) {
    ...
    if (!dateRange.value.includes(newDateStr)) {
        toast.error('Date outside calendar range');
        return;
    }
    ...
    if (!validateMachineDaySlot(newMachineId, newDateStr, [actualAlloc.key])) {
        toast.error('This machine already has an allocation on this date');
        return;
    }
    ...
    if (pd.type === 'workload') {
        ...
        if (forcedPlan.length > 0) {
            ...
            if (hasOverlap && overlapAffected.length > 0) {
                ...
                if (outOfBounds) {
                    toast.error('Cannot shift: some allocations would be pushed beyond the calendar date range. Extend end date first.');
                    return;
                }
        ...
        if (allocationsMade > 0) {
            ...
            if (remainingQty > 0) {
                ...
                toast.success(`Allocated ${qty} units across ${allocationsMade} day(s)`);
            }
        } else {
            toast.error('No capacity available');
        }

    } else if (pd.type === 'move') {
        const sourceAlloc = allocations.value.find(a => a.key === pd.allocKey);
        if (!sourceAlloc) {
            toast.error('Source allocation not found');
            closeDropModal();
            return;
        }
        ...
        if (qty > pd.availableCapacityQty) {
            ...
            if (allocationsMade > 0 || sourceResult.sourceRemoved) {
                ...
                toast.success(`Moved ${qty - remainingQty} units across ${allocationsMade} day(s)`);
            }
        } else {
            ...
            if (qty === sourceAlloc.quantity) {
                ...
                toast.success(`Moved ${qty} units, ${sourceAlloc.quantity} remain in source`);
            }
        }
    }
...
function confirmShift() {
    ...
    if (sd.type === 'move_group') {
        ...
        toast.success(`Shifted ${sd.affectedAllocations.length} allocation(s) and moved group successfully`);
        closeShiftModal();
        return;
    }
...
function executeShiftAndAllocate(shiftData, sourceResult) {
    ...
    if (allocationsMade > 0) {
        toast.success(`Shifted ${shiftCount} allocation(s). Allocated ${dropQty - remainingQty} units across ${allocationsMade} day(s).`);
    } else {
        toast.warn('Shift completed but no new allocations could be made');
    }
}
...
async function onOrderChange() {
    ...
    try {
        const data = await apiGetOrderData(selectedOrder.value);
        if (data) orderData.value = data;
    } catch (e) {
...
async function loadShiftAllocations() {
    try {
        ...
        if (data) {
            shiftAllocations.value = data.calendars;
            defaultAllocation.value = data.default_calendar;
        }
    } catch (e) {
...
async function loadAllAllocations() {
    try {
        const data = await apiGetAllAllocations(startDate.value, endDate.value);
        if (data && data.length > 0) {
            allocations.value = data.map((a, idx) => ({
                key: `existing-${idx}`,
                ...a
            }));
        } else {
...
async function loadShiftAllocationsForRange(rangeStart, rangeEnd) {
    try {
        const data = await apiGetShiftAllocations(rangeStart, rangeEnd);
        if (data) {
            const existingNames = new Set(shiftAllocations.value.map(c => c.name));
            const newCalendars = (data.calendars || []).filter(c => !existingNames.has(c.name));
            if (newCalendars.length > 0) {
                shiftAllocations.value = [...shiftAllocations.value, ...newCalendars];
            }
            if (!defaultAllocation.value && data.default_calendar) {
                defaultAllocation.value = data.default_calendar;
            }
        }
    } catch (e) {
...
async function loadAllocationsForRange(rangeStart, rangeEnd) {
    try {
        const data = await apiGetAllAllocations(rangeStart, rangeEnd);
        if (data && data.length > 0) {
            const existingNames = new Set(allocations.value.map(a => a.name));
            const newAllocs = data
...
async function doSaveAllocations() {
    validationErrors.value = [];
    const errors = [];
    ...
    if (allocations.value.length === 0) {
        toast.warn('No allocations to save');
        return;
    }

    try {
        const result = await apiSaveAllocations(allocations.value, startDate.value, endDate.value);
        if (result) {
...
async function confirmAlteration() {
    ...
    try {
        await apiAddShiftAlteration(form.date, form.alteration_type, form.minutes, form.machine || null, form.reason || null);
        toast.success('Shift alteration added');
        ...
        toast.error('Error adding alteration');
        alterationSaving.value = false;
    }
}
...
async function saveAlteration(idx) {
    ...
    try {
        await apiUpdateShiftAlteration(row.name, row.alteration_type, row.minutes, row.reason || null);
        ...
        toast.error('Error updating alteration');
    }
    editAlterationsSaving.value = false;
}
...
async function deleteAlteration(idx) {
    ...
    editAlterationsSaving.value = true;
    try {
        await apiDeleteShiftAlteration(row.name, row.parent);
        ...
        toast.error('Error deleting alteration');
    }
    editAlterationsSaving.value = false;
}
...
function openBulkShiftModal() {
    ...
    const defaultDate = dateRange.value.length > 0 ? dateRange.value[0] : new Date().toISOString().slice(0, 10);
...
async function applyBulkShiftUpdates() {
    ...
    try {
        ...
        for (const row of affected) {
            await apiAddShiftAlteration(dateStr, row.alteration_type, row.update_minutes, row.machine_id, null);
        }
        ...
        toast.success(`Shift updated for ${affected.length} machine(s)`);
        closeBulkShiftModal();
    } catch (e) {
        console.error('Error applying bulk shift updates:', e);
        toast.error('Error applying shift updates');
        bulkShiftSaving.value = false;
    }
}
...
async function confirmShiftUpdate() {
    ...
    try {
        const result = await apiUpdateDateShift(form.date, form.shifts, form.machine || null);
        ...
        if (result) {
            ...
            console.log('[ShiftUpdate] date:', form.date, 'delta:', delta,
                'result:', result,
                'isDefaultWeekdayOff:', isDefaultWeekdayOff(form.date),
                'new_minutes:', result?.new_minutes);
            if (delta > 0) {
...
async function loadAllShifts() {
    try {
        const data = await apiGetAllShifts();
        if (data) allShifts.value = data;
    } catch (e) {
        console.error('Error loading shifts:', e);
    }
}
...
onMounted(async () => {
    await nextTick();
    // Load master data (machines, processes, settings) in parallel
    const [machinesData, processesData] = await Promise.all([
        apiGetMachines().catch(() => []),
        apiGetProcesses().catch(() => []),
    ]);
    if (machinesData) machines.value = machinesData;
    if (processesData) processes.value = processesData;
    callMethod('frappe.client.get_value', { doctype: 'Albion Settings', fieldname: 'apply_zoom' }).then(val => {
        applyZoom.value = !!(val && val.apply_zoom);
    }).catch(() => {});
    await loadShiftAllocations();
    loadAllShifts();
    loadAllAllocations();
    ...
    if (!defaultAllocation.value) {
        toast.warn('No Default Shift Allocation', 'Please create a Shift Allocation and mark it as default for capacity planning to work correctly.');
    }
...
onBeforeRouteLeave((to, from, next) => {
    if (hasUnsavedChanges()) {
        confirm.require({
            ...
            acceptLabel: 'Leave',
            rejectLabel: 'Stay',
            accept() { actionHistory.value = []; redoHistory.value = []; next(); },
            reject() { next(false); }
        });
    } else {
        next();
